        ...
        else if (!_doMultiMaterial)
        {
            ...
            for (int i = 0; i < objsToMesh.Count; i++)
            {
                ...
                for (int j = 0; j < ms.Length; j++)
                {
                    ...
                    {
                      //  Debug.LogWarning("Game object " + objsToMesh[i] + " does not use shader " + targShader + " it may not have the required textures. If not small solid color textures will be generated."+"m.shader:"+m.shader);
                    }
