...
public class MB_TextureCombinerRenderTexture{
    ...
    public Texture2D DoRenderAtlas(GameObject gameObject, int width, int height, int padding, Rect[] rss, List<MB_TexSet> textureSetss, int indexOfTexSetToRenders, ShaderTextureProperty texPropertyname, MB3_TextureCombinerNonTextureProperties resultMaterialTextureBlender,  bool isNormalMap, bool fixOutOfBoundsUVs, bool considerNonTextureProperties, MB3_TextureCombiner texCombiner, MB2_LogLevel LOG_LEV){
		...
		_destinationTexture = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32);
		_destinationTexture.filterMode = FilterMode.Point;
	    _destinationTexture.autoGenerateMips = false;
	    _destinationTexture.useMipMap = SystemInfo.npotSupport == NPOTSupport.Full;
        ...
        //Bug #15425 windows Texture error
	...
	public void OnRenderObject(){
		if (_doRenderAtlas){
			...
			*/
			Texture2D tempTexture = new Texture2D(_destinationTexture.width, _destinationTexture.height, TextureFormat.ARGB32, _destinationTexture.mipmapCount, false);
	...
	{
		if (_destinationTexture.useMipMap)
			_destinationTexture.GenerateMips();
#if DEVELOPMENT_BUILD || UNITY_EDITOR
		Debug.Log($"Copy support: {SystemInfo.copyTextureSupport}");
#endif	
		var cpuCopy = (SystemInfo.copyTextureSupport & CopyTextureSupport.RTToTexture) == CopyTextureSupport.None;
		if (!cpuCopy)
		{
			//GPU copy
			Graphics.CopyTexture(_destinationTexture, 0, tempTexture, 0);
		}
		else
		{
			//Slow cpu copy
			...
			RenderTexture.active = _destinationTexture;
			tempTexture.ReadPixels(new Rect(0,0,_destinationTexture.width, _destinationTexture.height), 0, 0, false);
			tempTexture.Apply(tempTexture.mipmapCount > 1, true);
			RenderTexture.active = prev;
		}
		
		//Don't read texture contents, please.
		/*
