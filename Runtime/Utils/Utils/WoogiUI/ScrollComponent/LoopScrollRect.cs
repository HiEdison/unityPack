...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
            ...
            get
            {
                ...
                if (content != null)
                {
                    ...
                    if (layout2 != null)
                    {
                        ...
                        {
                            WDebug.LogWarning("[LoopScrollRect] Flexible not supported yet");
                        }
        ...
        public void SrollToCell(int index, float speed)
        {
            ...
            {
                WDebug.LogWarningFormat("invalid index {0}", index);
                return;
            }
            if (speed <= 0)
            {
                WDebug.LogWarningFormat("invalid speed {0}", speed);
                return;
            }
            StopAllCoroutines();
        ...
        protected RectTransform GetFromTempPool(int itemIdx)
        {
            ...
            {
                nextItem = GetObjectFromPool().transform as RectTransform;
        ...
        protected void ClearTempPool()
        {
            ...
            {
                deletedItemTypeStart--;
                ReturnObject(content.GetChild(0));
            }
            while (deletedItemTypeEnd > 0)
            {
                deletedItemTypeEnd--;
                ReturnObject(content.GetChild(content.childCount - 1));
            }
        }
        //==========LoopScrollRect==========
        ...
        private void UpdateBounds(bool updateItems = false)
        {
            ...
            if (Application.isPlaying && updateItems && UpdateItems(m_ViewBounds, m_ContentBounds))
            {
                Canvas.ForceUpdateCanvases();
                m_ContentBounds = GetBounds();
                UpdateBoundsSize(m_ContentBounds.size);
            }
            // ============LoopScrollRect============
        ...
        #region  custom
        public  abstract  void UpdateBoundsSize(Vector3 size);
        public abstract void ProvideData(Transform transform, int idx);
        public abstract void ReturnObject(Transform go);
        public abstract GameObject GetObjectFromPool(bool autoActive = true);
        #endregion
    }
}